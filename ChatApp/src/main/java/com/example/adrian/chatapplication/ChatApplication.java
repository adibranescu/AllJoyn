...
public class ChatApplication extends Application implements Observable {
    ...
    public PersonListWatcher personListWatcher;
    public IMWatcher messagesListWatcher;
    ...
    public synchronized void addFoundChannel(String channel) {
        ...
        if (personListWatcher != null) {

            (new Handler(Looper.getMainLooper ())).post(new Runnable () {
                @Override
                public void run () {
                    // notify person list watcher on main thread
                    personListWatcher.personListDidChange();
                }
            });
        }
    ...
    public synchronized void newLocalUserMessage(String message) {
        addInboundItem(hostGetChannelName(), message);
     ...
     */
    private void addHistoryItem(String nickname, final String message) {
        if (mHistory.size() == HISTORY_MAX) {
            mHistory.remove(0);
        }

        mHistory.add(message);
        if (messagesListWatcher != null) {

            (new Handler(Looper.getMainLooper ())).post(new Runnable () {
                @Override
                public void run () {
                    // notify messages list watcher on main thread
                    messagesListWatcher.didReceiveMessage(message);
                }
            });
        }
